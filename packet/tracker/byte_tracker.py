...
class STrack(BaseTrack):
    ...
    def tlwh_to_xywh(tlwh):
        ...
        """
        ret = np.asarray(tlwh).copy()
        ret[0] += ret[2] / 2  # center x = top-left x + (width / 2)
        ret[1] += ret[3] / 2  # center y = top-left y + (height / 2)
        return ret
...
class BYTETracker(object):
    def __init__(self, cfgs, img_size):
        ...
        # # print('match_thresh: {}'.format(args.match_thresh))
        ...
        self.gmc = GMC(self.cfgs, method=method, img_size=img_size)


    def update(self, output_results, img_size, img, frame=None, feature_map=None):
        self.frame_id += 1
        activated_stracks = []
        refind_stracks = []
        lost_stracks = []
        ...
        # CMC
        img = img.squeeze().permute(1, 2, 0)  # [1, 3, 800, 1440] -> [800, 1440, 3]
        img = torch.clamp(img, 0, 1)
        img = (255 * img).byte()
        #img = img.cpu().numpy()
        warp = self.gmc.apply(img, dets)  # time: 20ms
        ...
        if len(strack_pool) > 0 and len(detections) > 0:
            ...
            detect_features = self.get_feature_list_strack(detections, feature_map=feature_map)
        ...
        # print('appearance cost cost: ', (end_time - start_time) * 1000)
            ...
            strack_features, detect_features = None, None
        dists = matching.iou_distance(strack_pool, detections)
        if not self.cfgs.MOT20:
            dists = matching.fuse_score(dists, detections)
        # calc feauture cost matric
        matches, u_track, u_detection = matching.linear_assignment_appear(dists, self.cfgs.FIRST_MATCH_THRESH, strack_features, detect_features) # default 0.9
        # matches, u_track, u_detection = matching.linear_assignment(dists, self.args.match_thresh)
