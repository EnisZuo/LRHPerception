...
class GRU_CVAE(nn.Module):
    def __init__(self, cfg, dataset_name=None):
        ...
        self.dec_traj_output = nn.Linear(self.cfg.DEC_HIDDEN_SIZE, self.cfg.DEC_OUTPUT_DIM)
    ...
    def forward(self, input_x, 
        ...
        # pred_goal = pred_traj[:, -1, :, :].view(-1, 1, pred_traj.shape[-2], pred_traj.shape[-1])
        pred_goal = pred_goal + cur_pos
        pred_traj = pred_traj + cur_pos.unsqueeze(1)
        # 5. compute loss
        if target_y is not None:
            # train and val
            loss_goal, loss_traj = rmse_loss(pred_goal, 
                                            pred_traj, 
                                            target_y, 
    ...
    def pred_future_traj(self, dec_h, G):
        ...
        for t in range(pred_len): # the last step is the goal, no need to predict
            ...
            forward_input = self.traj_dec_input_forward(forward_h)
            output = self.dec_traj_output(forward_h)
            outputs.append(output.view(-1, K, output.shape[-1]))
        outputs.append(G)
        outputs = torch.stack(outputs, dim=1)
        # assert outputs.shape == (128, pred_len, K, G.shape[-1]), f"{outputs.shape}"
        return outputs